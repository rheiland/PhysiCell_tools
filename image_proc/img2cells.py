# 
# Map pixels of a given image into 2D hex-packed cell locations of a mesh domain.
#  
...
for yval in np.arange(ymin,ymax,ydel):
	...
	for xval in np.arange(xmin+xdel,xmax+xdel,cell_spacing):
		...
		if idx > img_xrange:
			continue
		if ((img[idx,idy,0] < 20) and (img[idx,idy,1] < 20)):  # black-ish
			xlist.append(yval)
			ylist.append(-xval)
			...
			fp.write(cell_str)
			num_cells += 1
		elif ((img[idx,idy,0] > 200) and (img[idx,idy,1] < 20)):  # red-ish
			xlist.append(yval)
			ylist.append(-xval)
			rlist.append(cell_radius)

...
class pyMCDS:
    ...
    def _read_xml(self, xml_file, output_path='.'):
        ...
        for label in cell_node.find('labels').findall('label'):
            ...
            if int(label.get('size')) == 100:
                data_labels.append(fixed_label)
                flag_vector_data[fixed_label] = True
                ...
                if int(label.get('size')) == 3:
                    ...
                    for i in range(int(label.get('size'))):
                        ...
                        flag_vector_data[ fixed_label + dir_label[i] ] = False
                else:
                    print("---- found custom data vector size = ",int(label.get('size')))
            else:
                data_labels.append(fixed_label)
                flag_vector_data[fixed_label] = False
        ...
        for col in range(len(data_labels)):
            if not flag_vector_data[ data_labels[col] ]: 
                MCDS['discrete_cells'][data_labels[col]] = cell_data[col, :]
            else:
                # 100 is vector size
                MCDS['discrete_cells'][data_labels[col]] = cell_data[col:col+100, :]
